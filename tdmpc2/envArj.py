# envArj.py
# Kinematic LIDAR navigation environment (TD-MPC2 compatible)
# Returns observations as a dict: {"state": tensor([lidar(120), dist_to_goal, angle_to_goal])}
# Action: 2-dim continuous in [-1, 1]: [v_norm, w_norm] where v = v_norm * max_speed, w = w_norm * max_turn
# Author: assistant (rewritten)
...
class RobotEnv:
    def __init__(self, gui=False, seed=None):
        self.gui = gui
        # Connect / reuse connection if available
        if p.isConnected():
        ...
        self.dt = 1.0 / 10.0         # control timestep: 0.1s. Keep consistent with training assumptions.
        ...
        self._obs_space = spaces.Box(low=-np.inf, high=np.inf, shape=(122,), dtype=np.float32)
    ...
    def reset(self, seed=None):
        ...
        if os.path.exists(urdf_path):
            self.robot = p.loadURDF(urdf_path, [self.start[0].item(), self.start[1].item(), self.robot_z])
        else:
            # small cube visual body
            self.robot = p.loadURDF("r2d2.urdf", [self.start[0].item(), self.start[1].item(), self.robot_z])

        # visualization of goal
        try:
            self.goal_vis = p.loadURDF("sphere_small.urdf", [self.goal[0].item(), self.goal[1].item(), 0.08])
        except Exception:
            self.goal_vis = p.loadURDF("cube.urdf", [self.goal[0].item(), self.goal[1].item(), 0.08], globalScaling=0.15)

        # Pose state (x, y, yaw) stored separately for kinematic integration
        self._pose = np.array([self.start[0].item(), self.start[1].item(), 0.0], dtype=np.float32)
    ...
    def step(self, action):
        """
        action: numpy array or torch tensor of shape (2,) with values in [-1,1]
                action[0] -> normalized linear velocity command
                action[1] -> normalized angular velocity command
        returns: obs_dict, reward_tensor, done_bool, info
        """
        self.step_count += 1
        ...
        # out of bounds
        elif abs(self._pose[0]) > self.arena_limit or abs(self._pose[1]) > self.arena_limit:
    ...
    def _get_obs(self):
        """Return dict {'state': tensor([lidar(120) normalized 0..1, dist_to_goal (m), angle_to_goal (rad)])}"""
        lidar = self._get_lidar()             # shape (num_rays,) as torch tensor normalized 0..1 (1.0 = max_dist no hit)
        dist_to_goal = self._goal_distance()  # torch scalar (meters)
        angle_to_goal = self._goal_angle()    # torch scalar (radians)

        state = torch.cat([lidar, dist_to_goal.view(1), angle_to_goal.view(1)]).float()  # shape (122,)
        return {"state": state}
    ...
    def _get_lidar(self):
        """
        Returns torch.tensor of length self.num_rays with normalized hit fractions:
        hit_fraction in [0,1], where 1.0 means no hit within max_dist.
        We use rayTestBatch for efficiency.
        """
        ...
        results = p.rayTestBatch(starts, ends)  # list of (hitObjectUid, linkIndex, hitFraction, hitPos, hitNormal)
    ...
    def _apply_pose_to_pybullet(self):
        """Apply the kinematic pose to the pybullet robot (visual only)"""
        ...
        try:
            if hasattr(self, "goal_vis"):
                p.resetBasePositionAndOrientation(self.goal_vis, [self.goal[0].item(), self.goal[1].item(), 0.08], p.getQuaternionFromEuler([0,0,0]))
        except Exception:
            pass

    def _create_walls(self):
        """Optional walls for bounding the arena (visual only)"""
        scale = 5
        # We'll place four large cubes as walls; scaled to be invisible to robot rays as needed
        try:
...
if __name__ == "__main__":
    ...
    try:
        while not done and steps < 200:
            ...
            if steps % 10 == 0:
                print(f"step={steps}, reward={reward.item():.3f}, min_lidar={float(obs['state'][:-2].min()):.3f}, dist={obs['state'][-2].item():.3f}")
            steps += 1
